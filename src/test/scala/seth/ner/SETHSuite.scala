...
class SingleTest extends AnyFunSuite {
  ...
  test("VerspoorPaper"){
    ...
    // Questionable cases:
    //missing amino acids
    assert(SETH.isValid("p.556_871dup", SETH.mutation, debug) === false)
    assert(SETH.isValid("p.264_265insSSGNL", SETH.mutation, debug) === false)
    assert(SETH.isValid("p.(550_592)ins7", SETH.mutation, debug) === false)
    //too unspecific? no nomenclature found
    assert(SETH.isValid("p.", SETH.mutation, debug) === false)
    assert(SETH.isValid("p.?", SETH.mutation, debug) === false)
    assert(SETH.isValid("p.?fs", SETH.mutation, debug) === false)
    assert(SETH.isValid("p.?del", SETH.mutation, debug) === false)
    assert(SETH.isValid("p.fs*?", SETH.mutation, debug) === false)
    assert(SETH.isValid("p.fs?", SETH.mutation, debug) === false)
    assert(SETH.isValid("p.?fs*?", SETH.mutation, debug) === false)
    ...
    // meaning of + AND - unclear
    assert(SETH.isValid("c.236+-6delAGCGAAGGT", SETH.mutation, debug) === false)
