...
case class EqString(override val parse: Any) extends ParsedString(parse) with MutationType
...
class SETH extends RegexParsers with NonGreedy with Positional with FlattenToMutation with PackratParsers {
  ...
  lazy val ProteinEq:P         = AALoc ~ ("=" ^^ { EqString(_) })
...
trait FlattenToMutation extends FlattenToString {
  ...
  def flattenToMutation(span: Span[Any]): Mutation = {
    ...
    for (att <- attributes) att match {
      ...
      case typ:EqString => mutation.typ = SILENT
